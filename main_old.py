...
def chunk_pdf(file_name_with_path):
    pages_and_texts = open_and_read_pdf(file_name_with_path)
    # The workflow which we are following:
    # Ingest text -> split it into groups/chunks -> embed the groups/chunks -> use the embeddings
    # random_pages_and_texts = random.sample(pages_and_texts, k=3)
...
def create_collection(name, dim, id_field, vector_field) -> Collection:
...
def create_index(collection, field_name):
    ...
    index = collection.create_index(field_name, index_param)
...
def main():
    ...
    # Finding embedding size to calculate the size of Milvus collection
    dim = len(pages_and_chunks[0]["embedding"])
    print(dim)
    # print(pages_and_chunks)
    # Create Collection, passing the embedding dimension to dynamically create collection with passed dim
    ...
    collection.flush()
    print('Records inserted to Milvus!', res)
    ...
    vector_store = MilvusVectorStore(collection_name=_COLLECTION_NAME,
                                     overwrite=False, )
    print('MilvusVectorStore created')
    ...
    response = query_engine.query("What file is this?")
'''
